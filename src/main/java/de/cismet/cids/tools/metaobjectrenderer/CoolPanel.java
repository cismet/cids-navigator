...
public class CoolPanel extends CustomMetaObjectRenderer implements ComponentListener,DoNotWrap {
     ...
     */
    public CoolPanel(final double geoBuffer) {
        this();
            ...
            swms.addRetrievalListener(new RetrievalListener() {
                ...
                public void retrievalComplete(final RetrievalEvent retrievalEvent) {
                    ...
                    if (o instanceof Image) {
                        map = (Image) o;
                        final BufferedImage erg = new BufferedImage(map.getWidth(null), map.getHeight(null), IMAGE_TYPE);
...
//            }
    ...
    /**
     * Setzt den relevanten Kartenausschnitt. Hierbei wird der Ausschnitt automatisch
     * weichgezeichnet. Um diese Karte im CoolPanel zu verwenden sollte er mit der
     * {@link getBlurredMap()}-Methode aufgerufen werden.
     * @param newMap der geaenderte zu blurrende Kartenauschnitt
     */
    private void createBackground(BufferedImage newMap) {
        if (newMap != null) {
            orgMap = newMap;
                ...
                public void run() {
                    ...
                    if (getPanMap() != null) {
                        ...
                        if ((bounds.width - 3 * offset) > orgMap.getWidth()) {
                            orgMap = orgMap.getSubimage(bounds.x, bounds.y + 2 * offset, orgMap.getWidth(), bounds.height);
                        } else {
                            orgMap = orgMap.getSubimage(bounds.x, bounds.y + 2 * offset, bounds.width - 3 * offset, bounds.height);
                        }
                    }
    ...
    private void mapIt() {
        ...
        try {
            ...
            if (geometry != null) {
                ...
                if (getPanMap() != null) {
                    double panWith = new Integer(getWidth() - 3 * offset).doubleValue();
                    double panHeight = new Integer(getHeight() - 4 * offset).doubleValue();
                    double cutOutWidth = new Integer(getPanMap().getWidth() - offset).doubleValue();
                    double cutOutHeight = new Integer(getPanMap().getHeight() - offset).doubleValue();
                    ...
                    double widthToHeightRatio = cutOutWidth / cutOutHeight;
                    ...
                    bb = new BoundingBox(midX - realWorldWidth / 2,
                            midY - realWorldHeight / 2,
                            midX + realWorldWidth / 2,
                            midY + realWorldHeight / 2);
                }
    ...
    /**
     * Setzt das Geometry-Objekt des momentan im Konfigurator angewaehlten Objekts.
     * Wird benoetigt, um einen Kartenhintergrund zu zeichnen.
