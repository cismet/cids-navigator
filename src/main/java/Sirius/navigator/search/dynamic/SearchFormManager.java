...
public class SearchFormManager extends javax.swing.JPanel {
    ...
    public SearchFormManager(final Map searchOptionsMap) {
         ...
         *
         * Iterator iterator = this.searchOptionsMap.keySet().iterator(); while(iterator.hasNext()) {     String queryId =
         * iterator.next().toString();     StringBuffer buffer = new StringBuffer("\nquery id: ").append(queryId);
         * SearchOption searchOption = this.getSearchOption(queryId);     Iterator parameterIterator =
         * searchOption.getParameterNames();     while(parameterIterator.hasNext())     { buffer.append("\nparameter
         * name: ");         buffer.append(parameterIterator.next());     }
         * buffer.append('\n');     logger.debug(buffer); } }*/
    ...
    protected java.util.List getSelectedSearchForms() {
         ...
         * while(iterator.hasNext()) { SearchForm searchForm =
         * ((SearchFormContainer)(SearchFormContainer)iterator.next()).getSelectedSearchForm();  if(searchForm != null)
     ...
     *
     * SearchFormFactory sff = new SearchFormFactory();     java.util.List searchFormsContainers =
     * sff.createSearchForms("D:\\work\\web\\Sirius\\navigator\\search\\", "search.xml", new SearchContext());
     *
     *
     * final SearchFormManager sfcm = new SearchFormManager(new HashMap(), searchFormsContainers);
     *
     * JFrame jf = new JFrame("SearchFormContainerManager");     jf.setDefaultCloseOperation(jf.EXIT_ON_CLOSE);
     ...
     *
     * LinkedHashMap parameterMap = new LinkedHashMap(); parameterMap.put("y0x0", "rechtsoben"); parameterMap.put("y1x2",
     * "linksunten"); FormDataBean formDataBean = new DefaultFormDataBean(parameterMap);
     * formDataBean.setBeanParameter("rechtsoben", "200"); formDataBean.setBeanParameter("linksunten", "400");
     *
     * SearchForm searchForm = new DefaultSearchForm("Planquadrat", formDataBean, Locale.getDefault());
     *
     * try { searchForm.readBeanParameters(); } catch(Exception exp) { sfcm.logger.fatal(exp.getMessage(), exp); }
     *
     * //searchForms.add(new DefaultSearchForm(new Query())); searchForms.add(searchForm);
     *
     * dsfc.setSearchForms(searchForms); LinkedList searchFormContainers = new LinkedList();
     * searchFormContainers.add(dsfc);sfcm.setSearchFormContainers(searchFormContainers);*/

    /*
     *      jf.pack();     jf.setVisible(true); } catch(Exception exp) {
     * Logger.getLogger(SearchFormManager.class).fatal(exp.getMessage(), exp);     System.exit(1); }}  */
}
