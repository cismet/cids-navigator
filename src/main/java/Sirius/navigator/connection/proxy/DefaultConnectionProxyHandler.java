...
//                        this.putMethods(methodMap);
        ...
        public MetaClass getCachedClass(User user, int classID, String localServerName) throws ConnectionException {
            ...
            if (!lsNames.contains(localServerName)) {
                ...
                if(log.isDebugEnabled())log.debug("<CC> Classes von neuem LocalServer " + localServerName + " gecacht");  // NOI18N
            }
        ...
        protected void putClasses(MetaClass[] classes, String localServerName) {
            ...
            for (int i = 0; i < classes.length; i++) {
                ...
                if(log.isDebugEnabled())log.debug("<CMC> Class gecacht: " + classes[i].getName() + " " + classes[i].getID() + " " + classes[i].getDomain());  // NOI18N
            }
            if(log.isDebugEnabled())log.debug("<CMC> " + classes.length + " Classes von LocalServer " + localServerName + " gecacht.");  // NOI18N
        }
        ...
        protected void putMethod(Sirius.server.localserver.method.Method method, String localServerName) {
            ...
            if (!methodHash.containsKey(key)) {
                ...
                if(log.isDebugEnabled())log.debug("<CMC> method '" + key + "' gecacht.");  // NOI18N
            }
        }
    ...
    class DefaultConnectionProxy implements ProxyInterface {
        public void setProperty(String name, String value) {
            if(log.isDebugEnabled())log.debug("[ProxyInterface] setting propety '" + name + "' to '" + value + "'");  // NOI18N
        }
        ...
        public MetaClass[] getClasses() throws ConnectionException {
            ...
            for(int i = 0; i < domains.length; i++) {
                ...
                try {
                    ...
                    log.error("Fehler im DefaultConnectionProxyHandler bei getClasses",t);  // NOI18N
...
//                throw new ConnectionException("could not load classes from localservers");
        ...
        public MetaObject getMetaObject(int objectID, int classID, String domain) throws ConnectionException {
            ...
            if(log.isDebugEnabled())
                log.debug(" MetaObject: " + MetaObject +" MetaObject.getName(): " + MetaObject.getName()+" MetaObject.getEditor(): " + MetaObject.getEditor()+" MetaObject.getComplexEditor(): " + MetaObject.getComplexEditor());  // NOI18N
...
//
        ...
        public MetaObject[] getMetaObject(Query query) throws ConnectionException {
            ...
            for(int i=0;i<obs.length;i++)
                if(obs[i]!=null)obs[i].setAllClasses(classAndMethodCache.getClassHash());
        ...
        public MetaObject[] getMetaObjectByQuery(String query,int sig) throws ConnectionException {
           if (classAndMethodCache==null) initClassAndMethodCache();
           if(log.isDebugEnabled())
            ...
            try {
                ...
                log.warn("Fehler in getMetaObjectByQuery",t);  // NOI18N
            }
            return null;
        }
...
//            return MetaObject;
        ...
        public Sirius.server.dataretrieval.DataObject getDataObject(MetaObject MetaObject) throws ConnectionException, Sirius.server.dataretrieval.DataRetrievalException {
            return connection.getDataObject(session.getUser(), MetaObject);
        }
        
        public Sirius.server.dataretrieval.DataObject[] getDataObject(Query query) throws ConnectionException, Sirius.server.dataretrieval.DataRetrievalException {
            return connection.getDataObject(session.getUser(), query);
        }
        ...
        /**
         * Sortiert und Gruppiert ein Node Array nach mehreren Kriterien.
         */
        
        /*protected void sortNodes(Node[] nodes, String[] criteria, boolean ascending) throws ConnectionException
        {
            int lastIndex = 0;
         
            // Ist z.Z. fuer alle Nodes gleich und wird durc das Model bestimmt
            ascending = PropertyManager.getManager().isSortAscending();
         
            if(criteria == null || criteria.length < 1)
                return;
         
            quicksortNodes(nodes, 0, nodes.length - 1, criteria[0], ascending);
            ...
            for(int i = 1; i < criteria.length; i++)
            {
                ...
                {
                    lastIndex = j;
                    String[] lastCriteria = new String[i];
                    System.arraycopy(criteria, 0, lastCriteria, 0, i);
         
                    while(j < nodes.length && equalsNode(nodes[j], nodes[j-1], lastCriteria))
                        ++j;
         
                    if(j - lastIndex > 0)
                    {
                        //NavigatorLogger.printMessage("Gruppenwechsel: " + (lastIndex - 1) + " : " + (j - 1) + " | " + criteria[i-1]);
                        quicksortNodes(nodes, lastIndex - 1, j - 1, criteria[i], ascending);
                    }
                }
            }
        }*/
        ...
        /**
         * Sortiert ein Node Array nach einem Kriterium.
         */
        
        /*protected void quicksortNodes(Node[] nodes, int lo0, int hi0, String criterion, boolean ascending) throws ConnectionException
        {
            //NavigatorLogger.printMessage("quicksortNodes: " + criterion + ".");
            ...
            if ( hi0 > lo0)
            {
                Node midNode = nodes[(lo0 + hi0) / 2];
         
                while (lo <= hi)
                {
                    if(ascending)
                    {
                        while (lo < hi0  && compareNodes(nodes[lo], midNode, criterion) < 0)
                            ++lo;
         
                        while (hi > lo0  && compareNodes(nodes[hi], midNode, criterion) > 0)
                            --hi;
                    }
                    else
                    {
            ...
            if( lo0 < hi )
                quicksortNodes(nodes, lo0, hi, criterion, ascending);
         
            if( lo < hi0 )
                quicksortNodes(nodes, lo, hi0, criterion, ascending);
        }*/
        
        /**
         * Diese Funktion vergleicht ein oder mehrere ein Attribute zweier Nodes und
         * gibt an, ob alle Attribute \u00FCbereinstimmen.
         *
         * @node Referenznode
         * @comparator Vergleichs-Node
         * @criteria Attribute, die verglichen werden soll
         * @return true oder false.
         */
        /*private boolean equalsNode(Node node, Node comparator, String[] criteria) throws ConnectionException
        {
            for(int i = 0; i < criteria.length; i++)
            {
                if(compareNodes(node, comparator, criteria[i]) != 0)
                    return false;
            }
        ...
        /**
         * Diese Funktion vergleicht genau ein Attribut zweier Nodes und
         * gibt an, ob dieses Attribut kleiner, grosser oder gleich dem
         * Attribut der anderen Node ist.
         *
         * @node Referenznode
         * @comparator Vergleichs-Node
         * @criterion Attribut, das verglichen werden soll
         * @return -1 = kleiner, 0 = gleich, 1 = groesser
         */
       /* private int compareNodes(Node node, Node comparator, String criterion) throws ConnectionException
        {
            if(criterion.equals(PropertyManager.SORT_NAME_TOKEN))
            {
                //NavigatorLogger.printMessage("<quicksort NAME> " + node.getName() + " compareTo " + comparator.getName() + " = " + node.getName().compareTo(comparator.getName()));
                //if(logger.isDebugEnabled())log.debug("<SORT> sorting by NAME");
                return node.getName().compareTo(comparator.getName());
            }
            else if(criterion.equals(PropertyManager.SORT_ID_TOKEN))
            {
                if(log.isDebugEnabled())log.debug("<SORT> sorting by ID");
        
                String nodeClassID = null;
                String comparatorClassID = null;
        
                if(node instanceof MetaObjectNode)
                {
                    nodeClassID = ((MetaObjectNode)node).getDomain() + ((MetaObjectNode)node).getObject().getClassID();
                }
                else if(node instanceof MetaClassNode)
                {
                    nodeClassID = ((MetaClassNode)node).getDomain() + ((MetaClassNode)node).getClassID();
                }
                else
                {
                    //return 1;
                    nodeClassID = String.valueOf(node.getID());
                }
        
                if(comparator instanceof MetaObjectNode)
                {
                    comparatorClassID = ((MetaObjectNode)comparator).getDomain() + ((MetaObjectNode)comparator).getObject().getClassID();
                }
                else if(comparator instanceof MetaClassNode)
                {
                    comparatorClassID = ((MetaClassNode)comparator).getDomain() + ((MetaClassNode)comparator).getClassID();
                }
                else
                {
                    //return 1;
                    comparatorClassID = String.valueOf(comparator.getID());
                }
        
                return nodeClassID.compareTo(comparatorClassID);
            }
            else
            {
                criterion = criterion.substring(1, criterion.length()-1);
                //if(log.isDebugEnabled())log.debug("<SORT> sorting by attribute '" + criterion + "'");
        
                Sirius.server.localserver.attribute.Attribute[] nodeAttributes = null;
                Sirius.server.localserver.attribute.Attribute[] comparatorAttributes = null;
        
                //Navigatorlog.printMessage("<quicksort ATTRIBUTE>");
        
                if(node instanceof MetaObjectNode)
                {
                    nodeAttributes = ((MetaObjectNode)node).getObject().getAttribs();
                }
                else if(node instanceof MetaClassNode)
                {
                    nodeAttributes = this.getMetaClass(((MetaClassNode)node).getClassID(), node.getDomain()).getAttribs();
                }
                else
                {
                    //if(log.isDebugEnabled())log.warn("<SORT> could not sort nodes: no attributes available (node)");
                    return 1;
                }
        
                if(comparator instanceof MetaObjectNode)
                {
                    comparatorAttributes = ((MetaObjectNode)comparator).getObject().getAttribs();
                }
                else if(comparator instanceof MetaClassNode)
                {
                    comparatorAttributes = getMetaClass(((MetaClassNode)comparator).getClassID(), comparator.getDomain()).getAttribs();
                }
                else
                {
                    //if(log.isDebugEnabled())log.warn("<SORT> could not sort nodes: no attributes available (comparator)");
                    return 1;
                }
        
                for(int i = 0; i < nodeAttributes.length; i++)
                {
                    if(nodeAttributes[i].getName().equalsIgnoreCase(criterion))
                    {
                        for(int j = 0; j < comparatorAttributes.length; j++)
                        {
                            if(comparatorAttributes[j].getName().equalsIgnoreCase(criterion))
                            {
                                Object nodeAttribute = nodeAttributes[i].getValue();
                                Object comparatorAttribute = comparatorAttributes[i].getValue();
        
                                if(nodeAttribute != null && comparatorAttribute != null)
                                {
                                    if(Comparable.class.isAssignableFrom(nodeAttribute.getClass()) && Comparable.class.isAssignableFrom(comparatorAttribute.getClass()))
                                    {
                                        return(((Comparable)nodeAttribute).compareTo(comparatorAttribute));
                                    }
                                    else if(log.isDebugEnabled())
                                    {
                                        log.warn("<SORT> could not sort nodes: attribute values not compareable");
                                    }
                                }
                                else if(log.isDebugEnabled())
                                {
                                    log.warn("<SORT> could not sort nodes: null-attribute-value");
                                }
        
                                return 1;
        
                                //Navigatorlog.printMessage("<quicksort ATTRIBUTE> " + nodeAttributes[i].getValue() + " compareTo " + comparatorAttributes[j].getValue() + " = " + nodeAttributes[i].getValue().compareTo(comparatorAttributes[j].getValue()));
                                //return nodeAttributes[i].getValue().toString().compareTo(comparatorAttributes[j].getValue().toString());
                            }
                        }
                    }
                }
        
                //if(log.isDebugEnabled())log.warn("<SORT> could not sort nodes: attribute '" + criterion + "' not found");
                return 1;
            }
        }*/
    }
